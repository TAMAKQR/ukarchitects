            ...
            const settingsSections = [
                ...
                },
                {
                    ...
                    fields: [
                        ...
                        { key: 'website_url', label: 'Адрес сайта', type: 'url', description: 'Ссылка на дополнительный сайт компании' },
                    ]
                },
                {
...
async function loadHeroSlides() {
    try {
        const response = await authFetch(`${API_URL}/hero-slides`);
        const slides = await response.json();

        const tableBody = document.getElementById('hero-slides-table');
        tableBody.innerHTML = slides.map(slide => `
            <tr>
                ...
                <td>${slide.visible ? '✅ Да' : '❌ Нет'}</td>
                <td>
                    <button class="btn btn-sm" onclick="editHeroSlide(${slide.id})">Редактировать</button>
                    <button class="btn btn-sm btn-danger" onclick="deleteHeroSlide(${slide.id})">Удалить</button>
                </td>
            </tr>
        `).join('');
    } catch (error) {
        console.error('Ошибка загрузки hero слайдов:', error);
    }
}

function openHeroSlideModal(id = null) {
    console.log('openHeroSlideModal вызвана, id:', id);

    const modalHtml = `
        <div class="modal active" id="hero-slide-modal">
            <div class="modal-content" style="max-width: 800px;">
                <span class="close" onclick="closeModal('hero-slide-modal')">&times;</span>
                <h2>${id ? 'Редактировать' : 'Добавить'} Hero слайд</h2>
                <form id="hero-slide-form" onsubmit="saveHeroSlide(event, ${id})">
                    <div class="form-group">
                        <label>Тип медиа *</label>
                        <select id="slide-media-type" onchange="toggleMediaFields()">
                    ...
                    <div class="form-group">
                        <label>Описание</label>
                        <textarea id="slide-description" rows="3"></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label>Категория</label>
                        <input type="text" id="slide-category" placeholder="Например: Жилой комплекс">
                    </div>
                    ...
                    <div class="form-group" id="video-field" style="display: none;">
                        ...
                        <input type="url" id="slide-video-url" placeholder="Вставьте прямую ссылку на видео (опционально)" style="margin-top: 5px;">
                    </div>
                    
                    <div class="form-group">
                        <label>Текст кнопки</label>
                        <input type="text" id="slide-button-text" value="Подробнее">
                    </div>
                    
                    <div class="form-group">
                        <label>Ссылка кнопки</label>
                        <input type="text" id="slide-button-link" placeholder="/projects.html или https://...">
                    </div>
                    ...
                    <div class="form-group">
                        <label>
                            <input type="checkbox" id="slide-visible" checked>
                            Видимый
                        </label>
                    </div>
    ...
    console.log('Добавляем модальное окно в DOM');
    document.body.insertAdjacentHTML('beforeend', modalHtml);
    console.log('Модальное окно добавлено');

    if (id) {
        loadHeroSlideData(id);
    }
}

function toggleMediaFields() {
    const mediaType = document.getElementById('slide-media-type').value;
    ...
    if (mediaType === 'image') {
        ...
        document.getElementById('slide-video-file').required = false;
        document.getElementById('slide-video-url').required = false;
    } else {
...
async function loadHeroSlideData(id) {
    try {
        const response = await authFetch(`${API_URL}/hero-slides/${id}`);
        const slide = await response.json();

        document.getElementById('slide-media-type').value = slide.media_type || 'image';
        toggleMediaFields();
        ...
        document.getElementById('slide-video-url').value = slide.video_url || '';
        document.getElementById('slide-button-text').value = slide.button_text || 'Подробнее';
        document.getElementById('slide-button-link').value = slide.button_link || '';
        document.getElementById('slide-order').value = slide.order_num || 0;
        document.getElementById('slide-visible').checked = slide.visible;

        if (slide.image_url) {
            document.getElementById('slide-image-preview').innerHTML = `
                <img src="${slide.image_url}" style="max-width: 300px; border-radius: 8px;">
            `;
        }
    } catch (error) {
...
async function saveHeroSlide(event, id = null) {
    event.preventDefault();
    ...
    const orderNum = document.getElementById('slide-order').value;
    const visible = document.getElementById('slide-visible').checked ? 1 : 0;

    let imageUrl = null;
    let videoUrl = null;

    if (mediaType === 'video') {
        // Для видео можем загрузить файл или использовать URL
        ...
        if (imageFile) {
            ...
            // Если редактируем и изображение не меняли, получаем старое
            const response = await authFetch(`${API_URL}/hero-slides/${id}`);
            const slide = await response.json();
...
async function deleteHeroSlide(id) {
    ...
    try {
        await authFetch(`${API_URL}/hero-slides/${id}`, { method: 'DELETE' });
        loadHeroSlides();
        alert('Слайд удален');
    } catch (error) {
